...
class Fraccion
	...
	def initialize(*a) #Se pasan uno o mas argumentos
      ...
      raise ArgumentError, "La cadena introducida no concuerda con a/b o a siendo a y b enteros." if /\d+(\/\d+)?/.match(a[0]).to_s != a[0] #se comprueba que la cadena concuerda con uno de los tipos aceptacos "a/b" o a siendo a y b enteros.
 #Se comprueba que la cadena que se le pasa es del tipo a/b o a
			if /\d+\/\d+/.match(a[0])
        ...
        raise ArgumentError , "No se puede dividir por 0" if denominador == 0
			else
				numerador = ((/\d+/.match(a[0])).to_s).to_i # recojo el número antes de la /
				denominador = 1
			end
		...
		raise ArgumentError, "Se esperaba un entero" unless numerador.is_a?(Numeric) #Compruebo si a es un numero
		raise ArgumentError, "Se esperaba un entero" unless denominador.is_a?(Numeric) #Compruebo si b es un numero
	...
	def to_s  # devuelve una cadena con la representación del racional
		if @numerador == 0 # Si el numerador es 0 imprime solo cualquier numero divido por 0 da 0
			"0"
    elsif @denominador == 1 #Si es denominador 1 el resutaldo es el numerador
      "#{@numerador}"
